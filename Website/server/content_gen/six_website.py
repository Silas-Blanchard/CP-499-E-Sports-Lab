...
def do_GET():
    ...
    db_path, rect_csv_path, wall_csv_path, HTML_path, labels_path, decor_path = get_file_paths(BASE_DIR)

    # Database operations
    rows = fetch_database_data(db_path)

    # Read CSV files
    computers, walls, labels, decor = read_csv_files(rect_csv_path, wall_csv_path, labels_path, decor_path)

    # Generate HTML content with calendar events
    HTML_text = generate_html_content(rows, computers, walls, labels, decor)
...
def read_csv_files(rect_csv_path, wall_csv_path, label_csv_path, decor_csv_path):
    ...
    with open(decor_csv_path, "r") as file:
        decor = list(csv.reader(file, delimiter=','))
        decor = decor[1:]  # Skip header row
    
    return computers, walls, labels, decor


def generate_html_content(rows, computers, walls, labels, decor_path):
    ...
    HTML_text += generate_decor(decor_path)
...
def generate_labels(label_path):
    ...
    return outputHTML
#the labels
def generate_decor(decs):
    outputHTML = ""
    for i in decs:
        decor, x, y, width, height = i
        outputHTML += f"""
...
def generate_header():
    ...
    <div class="header">
        ...
        <a href="www.google.com" class="login-button">Login</a>
    </div>
    """
